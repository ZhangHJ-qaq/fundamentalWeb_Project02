...
class HTMLPurifier_Strategy_MakeWellFormed extends HTMLPurifier_Strategy
{
    ...
    public function execute($tokens, $config, $context)
    {
        ...
        for (;;
            ...
            if (is_int($i)) {
                ...
                if (is_int($rewind_offset)) {
                    for ($j = 0; $j < $rewind_offset; $j++) {
                        ...
                        // indicate that other injectors should not ajax this token,
            ...
            // quick-check: if it's not a tag, no need to ajax
            ...
            if ($type === 'empty' && $token instanceof HTMLPurifier_Token_Start) {
                ...
                if (!empty($this->stack)) {
                    ...
                    // stream and then /ajax/ that, before
...
//
// Thus, the idea of a skip is that once we ajax a token with
...
//    reprocessed.
//
//  - Suppose that you successfuly ajax a token, replace it with
//    one with your skip mark, but now another injector wants to
//    ajax the skipped token with another token.  Should you continue
